...
-- Widget setup reads secret text metrics (GetStringHeight and friends) and does arithmetic on
-- them, which throws under our taint. The mixins involved are shared across the whole widget
-- system, so this tooltip scoped entry point is the only place we may safely touch.
--
-- Verified in game that this wrapper does not affect world quest tooltips; the earlier breakage
-- came from the InsertFrame rewrite below, not from here.
